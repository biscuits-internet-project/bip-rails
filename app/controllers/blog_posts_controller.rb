class BlogPostsController < ApplicationController
  skip_before_action :authenticate_user!, only: %i[index show tags]
  before_action :authorize_admin, only: %i[create update destroy publish]
  before_action :set_blog_post, only: %i[show update destroy publish]
  ...
  def index
    ...
    begin
      posts = posts.state(params[:state] || 'published')

      posts = posts.joins(:user).where(users: { username: params[:username] }) if params[:username]
    end
